...
pub struct GetActiveForEmployeeRequest {
    pub tenant_id: StandardID<IDTenant>,
    pub employee_id: StandardID<IDEmployee>,
}

pub struct GetActiveForEmployeeResponse {
...
pub(super) async fn execute<S: CompensationStore>(
    ...
    let compensation_profile = svc
        .store()
        .get_active_for_employee(&req.tenant_id, &req.employee_id)
        .await
...
mod tests {
    ...
    async fn calls_store_with_tenant_and_employee() {
        ...
        store
            .expect_get_active_for_employee()
